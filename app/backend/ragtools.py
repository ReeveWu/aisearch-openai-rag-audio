...
async def _search_tool(search_client: SearchClient, args: Any) -> ToolResult:
    ...
    search_results = await search_client.search(
        ...
        top=5,
        vector_queries=[VectorizableTextQuery(text=args['query'], k_nearest_neighbors=50, fields="vector")],
        select="chunk_id,section_headers,content")
    result = ""
...
async def _report_grounding_tool(search_client: SearchClient, args: Any) -> None:
    ...
    docs = []
    async for r in search_results:
        docs.append({"chunk_id": r['chunk_id'], "title": r["section_headers"], "chunk": r['content']})
